        ...
        if (table === "dwg_files") {
          if (isInsert) {
            mockDwgFiles.push({
              ...
              version: args[3] || "1.0",
              size: args[4] || 0,
              mime_type: args[5] || "application/acad",
            });
            return { changes: 1 };
          }
          if (isUpdate) {
            const dwg = mockDwgFiles.find(
              (d) => d.id === args[args.length - 1],
            );
            if (dwg) {
        ...
        if (table === "mappings") {
          if (isInsert) {
            mockMappings.push({
              ...
              tag: args[4],
              task_field: args[5],
              planning_field: args[6],
            });
            return { changes: 1 };
          }
          if (isUpdate) {
            const mapping = mockMappings.find(
              (m) => m.id === args[args.length - 1],
            );
            if (mapping) {
      ...
      get: (...args) => {
        if (table === "users") {
          if (sql.includes("WHERE email ="))
            return mockUsers.find((u) => u.email === args[0]);
          if (sql.includes("WHERE id ="))
            return mockUsers.find((u) => u.id === args[0]);
          return mockUsers[0];
        }
        if (table === "projects")
          return mockProjects.find((p) => p.id === args[0]);
        if (table === "dwg_files") {
          if (sql.includes("WHERE id ="))
            return mockDwgFiles.find((d) => d.id === args[0]);
          return mockDwgFiles[0];
        }
        if (table === "tasks") {
          if (sql.includes("WHERE id ="))
            return mockTasks.find((t) => t.id === args[0]);
          return mockTasks[0];
        }
        if (table === "mappings")
          return mockMappings.find((m) => m.id === args[0]);
        if (table === "audit_logs")
          return mockAuditLogs.find((a) => a.id === args[0]);
        return undefined;
      },
      all: (...args) => {
        ...
        if (table === "dwg_files") {
          if (sql.includes("WHERE project_id ="))
            return mockDwgFiles.filter((d) => d.project_id === args[0]);
          return [...mockDwgFiles];
        }
        if (table === "tasks") {
          if (sql.includes("WHERE t.assigned_to"))
            return mockTasks.filter((t) => t.assigned_to === args[0]);
          if (
            sql.includes("WHERE t.project_id") ||
            sql.includes("WHERE project_id")
          )
            return mockTasks.filter((t) => t.project_id === args[0]);
          return [...mockTasks];
        }
        if (table === "mappings") {
          if (sql.includes("WHERE dwg_file_id ="))
            return mockMappings.filter((m) => m.dwg_file_id === args[0]);
          return [...mockMappings];
        }
        if (table === "audit_logs") {
          if (sql.includes("WHERE user_id ="))
            return mockAuditLogs.filter((a) => a.user_id === args[0]);
          if (
            sql.includes("WHERE resource_type =") &&
            sql.includes("resource_id =")
          )
            return mockAuditLogs.filter(
              (a) => a.resource_type === args[0] && a.resource_id === args[1],
            );
